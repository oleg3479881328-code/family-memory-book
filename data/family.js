window.FAMILY = {
  "people": {
    "alexander-fateev": {
      "children": [
        "konstantin-fateev",
        "pavel-fateev"
      ],
      "dates": "1930 – 2008",
      "name": "Александр Константинович Фатеев",
      "notes": [
        "Муж Александры Дмитриевны Куреневой."
      ],
      "parents": [],
      "partners": [
        "alexandra-kureneva"
      ]
    },
    "alexandra-kureneva": {
      "children": [
        "konstantin-fateev",
        "pavel-fateev"
      ],
      "dates": "29.12.1927 – 02.03.2011",
      "name": "Александра Дмитриевна Куренева",
      "notes": [
        "Жена Александра Константиновича Фатеева."
      ],
      "parents": [
        "anna-kuteinikova",
        "dmitry-kurenev"
      ],
      "partners": [
        "alexander-fateev"
      ]
    },
    "alexandra-pirina": {
      "children": [
        "anna-povalyukhina"
      ],
      "dates": "17.10.1970",
      "name": "Александра Сергеевна Пирина",
      "notes": [
        "Жена Олега Игоревича Повалюхина."
      ],
      "parents": [],
      "partners": [
        "oleg-povalyukhin"
      ]
    },
    "anatoly-povalyukhin": {
      "children": [],
      "dates": "15.02.1974 – 31.08.1995",
      "memoirs": [
        "семейные-хроники"
      ],
      "name": "Анатолий Игоревич Повалюхин",
      "notes": [
        "Сын Тамары Николаевны Повалюхиной и Игоря Петровича Повалюхина."
      ],
      "parents": [
        "igor-povalyukhin",
        "tamara-grigoryeva"
      ],
      "partners": []
    },
    "anna-fateeva": {
      "children": [],
      "dates": "10.07.1990",
      "name": "Анна Константиновна Фатеева",
      "notes": [
        "Дочь Константина Александровича Фатеева и Тамары Антоновны Жулёвой."
      ],
      "parents": [
        "konstantin-fateev",
        "tamara-zhuleva"
      ],
      "partners": []
    },
    "anna-kuteinikova": {
      "children": [
        "alexandra-kureneva",
        "vera-kureneva",
        "elizaveta-kureneva",
        "natalya-kureneva"
      ],
      "dates": "18.07.1899 – 02.02.1990",
      "memoirs": [
        "воспоминания-веры-дмитриевны-подковцевой",
        "русская-печь",
        "семейные-хроники"
      ],
      "name": "Анна Сергеевна Кутейникова",
      "notes": [
        "Дочь Сергея Ивановича и Марии Кутейниковых.",
        "Жена Дмитрия Андреевича Куренёва."
      ],
      "parents": [
        "maria-kuteinikova",
        "sergey-kuteinikov"
      ],
      "partners": [
        "dmitry-kurenev"
      ]
    },
    "anna-povalyukhina": {
      "children": [],
      "dates": "06.06.2001",
      "memoirs": [
        "семейные-хроники"
      ],
      "name": "Анна Олеговна Повалюхина",
      "notes": [
        "Дочь Олега Игоревича Повалюхина и Александры Сергеевны Пириной."
      ],
      "parents": [
        "alexandra-pirina",
        "oleg-povalyukhin"
      ],
      "partners": []
    },
    "daniil-fateev": {
      "children": [],
      "dates": "02.03.2017",
      "name": "Даниил Фатеев",
      "notes": [
        "Сын Никиты Павловича Фатеева и Дарьяны Сергеевны Медведевой."
      ],
      "parents": [
        "daryana-medvedeva",
        "nikita-fateev"
      ],
      "partners": []
    },
    "daryana-medvedeva": {
      "children": [
        "daniil-fateev"
      ],
      "dates": "22.04.1987",
      "name": "Дарьяна Сергеевна Медведева",
      "notes": [
        "Жена Никиты Павловича Фатеева."
      ],
      "parents": [],
      "partners": [
        "nikita-fateev"
      ]
    },
    "dmitry-kurenev": {
      "children": [
        "alexandra-kureneva",
        "vera-kureneva",
        "elizaveta-kureneva",
        "natalya-kureneva"
      ],
      "dates": "08.11.1895 – 13.02.1953",
      "memoirs": [
        "воспоминания-веры-дмитриевны-подковцевой",
        "семейные-хроники"
      ],
      "name": "Дмитрий Андреевич Куренёв",
      "notes": [
        "Муж Анны Сергеевны Кутейниковой."
      ],
      "parents": [],
      "partners": [
        "anna-kuteinikova"
      ]
    },
    "elena-podkovtseva": {
      "children": [],
      "dates": "05.11.1949",
      "memoirs": [
        "семейные-хроники"
      ],
      "name": "Елена Николаевна Подковцева",
      "notes": [
        "Дочь Николая Павловича Подковцева и Веры Дмитриевны Куреневой."
      ],
      "parents": [
        "vera-kureneva",
        "nikolay-podkovtsev"
      ],
      "partners": []
    },
    "elizaveta-kureneva": {
      "children": [],
      "dates": "умерла в 6 лет",
      "name": "Елизавета Дмитриевна Куренёва",
      "notes": [
        "Дочь Анны Сергеевны Кутейниковой и Дмитрия Андреевича Куренёва."
      ],
      "parents": [
        "anna-kuteinikova",
        "dmitry-kurenev"
      ],
      "partners": []
    },
    "gerard-hurley": {
      "children": [
        "hawkin-hurley"
      ],
      "name": "Джерард Хёрли",
      "notes": [
        "Муж Натальи Олеговны Повалюхиной."
      ],
      "parents": [],
      "partners": [
        "natalya-povalyukhina"
      ]
    },
    "hawkin-hurley": {
      "children": [],
      "dates": "11.11.2018",
      "name": "Хокин Рэндал Хёрли",
      "notes": [
        "Сын Натальи Олеговны Повалюхиной и Джерарда Хёрли."
      ],
      "parents": [
        "gerard-hurley",
        "natalya-povalyukhina"
      ],
      "partners": []
    },
    "igor-grigoryev": {
      "children": [],
      "dates": "20.11.1958 – 19.06.1993",
      "name": "Игорь Николаевич Григорьев",
      "notes": [
        "Сын Николая Дмитриевича Григорьева и Натальи Дмитриевны Куренёвой."
      ],
      "parents": [
        "natalya-kureneva",
        "nikolay-grigoryev"
      ],
      "partners": []
    },
    "igor-povalyukhin": {
      "children": [
        "anatoly-povalyukhin",
        "oleg-povalyukhin"
      ],
      "dates": "23.03.1947 – 21.05.2014",
      "memoirs": [
        "отец"
      ],
      "name": "Игорь Петрович Повалюхин",
      "notes": [
        "Муж Тамары Николаевны Повалюхиной."
      ],
      "parents": [
        "petr-povalyukhin",
        "vera-povalyukhina"
      ],
      "partners": [
        "tamara-grigoryeva"
      ]
    },
    "konstantin-fateev": {
      "children": [
        "anna-fateeva"
      ],
      "dates": "09.09.1955",
      "name": "Константин Александрович Фатеев",
      "notes": [
        "Муж Тамары Антоновны Жулёвой."
      ],
      "parents": [
        "alexander-fateev",
        "alexandra-kureneva"
      ],
      "partners": [
        "tamara-zhuleva"
      ]
    },
    "maria-kuteinikova": {
      "children": [
        "anna-kuteinikova"
      ],
      "dates": "даты неизвестны",
      "name": "Мария Кутейникова",
      "notes": [
        "Мать Анны Сергеевны Кутейниковой."
      ],
      "parents": [],
      "partners": [
        "sergey-kuteinikov"
      ]
    },
    "maria-povalyukhina": {
      "children": [],
      "dates": "11.02.2013",
      "name": "Мария Олеговна Повалюхина",
      "notes": [
        "Дочь Олега Игоревича Повалюхина и Ольги Александровны Таньковой."
      ],
      "parents": [
        "oleg-povalyukhin",
        "olga-tankova"
      ],
      "partners": []
    },
    "marina-drozdova": {
      "children": [
        "nikita-fateev"
      ],
      "dates": "23.09.1964",
      "name": "Марина Валерьевна Дроздова",
      "notes": [
        "Жена Павла Александровича Фатеева."
      ],
      "parents": [],
      "partners": [
        "pavel-fateev"
      ]
    },
    "natalya-kureneva": {
      "children": [
        "igor-grigoryev",
        "tamara-grigoryeva"
      ],
      "dates": "27.09.1921 – 08.02.1976",
      "memoirs": [
        "наташа",
        "семейные-хроники",
        "родители"
      ],
      "name": "Наталья Дмитриевна Куренёва",
      "notes": [
        "Жена Николая Дмитриевича Григорьева."
      ],
      "parents": [
        "anna-kuteinikova",
        "dmitry-kurenev"
      ],
      "partners": [
        "nikolay-grigoryev"
      ]
    },
    "natalya-povalyukhina": {
      "children": [
        "hawkin-hurley"
      ],
      "dates": "07.10.1989",
      "name": "Наталья Олеговна Повалюхина",
      "notes": [
        "Жена Джерарда Хёрли."
      ],
      "parents": [
        "vera-solovyova",
        "oleg-povalyukhin"
      ],
      "partners": [
        "gerard-hurley"
      ]
    },
    "nikita-fateev": {
      "children": [
        "daniil-fateev"
      ],
      "dates": "31.07.1987",
      "name": "Никита Павлович Фатеев",
      "notes": [
        "Муж Дарьяны Сергеевны Медведевой."
      ],
      "parents": [
        "marina-drozdova",
        "pavel-fateev"
      ],
      "partners": [
        "daryana-medvedeva"
      ]
    },
    "nikolay-grigoryev": {
      "children": [
        "igor-grigoryev",
        "tamara-grigoryeva"
      ],
      "dates": "19.12.1914 – 19.04.1994",
      "memoirs": [
        "родители"
      ],
      "name": "Николай Дмитриевич Григорьев",
      "notes": [
        "Муж Натальи Дмитриевны Куренёвой."
      ],
      "parents": [],
      "partners": [
        "natalya-kureneva"
      ]
    },
    "nikolay-podkovtsev": {
      "children": [
        "elena-podkovtseva"
      ],
      "dates": "24.12.1924 – 25.08.1991",
      "name": "Николай Павлович Подковцев",
      "notes": [
        "Муж Веры Дмитриевны Куреневой."
      ],
      "parents": [],
      "partners": [
        "vera-kureneva"
      ]
    },
    "oleg-povalyukhin": {
      "children": [
        "anna-povalyukhina",
        "maria-povalyukhina",
        "natalya-povalyukhina"
      ],
      "dates": "26.01.1970",
      "memoirs": [
        "семейные-хроники"
      ],
      "name": "Олег Игоревич Повалюхин",
      "notes": [
        "Сын Тамары Николаевны Повалюхиной и Игоря Петровича Повалюхина.",
        "Муж Веры Дмитриевны Соловьёвой, Александры Сергеевны Пириной и Ольги Александровны Таньковой."
      ],
      "parents": [
        "igor-povalyukhin",
        "tamara-grigoryeva"
      ],
      "partners": [
        "alexandra-pirina",
        "vera-solovyova",
        "olga-tankova"
      ]
    },
    "olga-tankova": {
      "children": [
        "maria-povalyukhina"
      ],
      "dates": "01.06.1983",
      "name": "Ольга Александровна Танькова",
      "notes": [
        "Жена Олега Игоревича Повалюхина."
      ],
      "parents": [],
      "partners": [
        "oleg-povalyukhin"
      ]
    },
    "pavel-fateev": {
      "children": [
        "nikita-fateev",
        "sasha-fateev"
      ],
      "dates": "11.08.1960",
      "name": "Павел Александрович Фатеев",
      "notes": [
        "Муж Марины Валерьевны Дроздовой.",
        "Муж Веры Николаевны Плиткиной."
      ],
      "parents": [
        "alexander-fateev",
        "alexandra-kureneva"
      ],
      "partners": [
        "vera-plitkina",
        "marina-drozdova"
      ]
    },
    "petr-povalyukhin": {
      "children": [
        "igor-povalyukhin"
      ],
      "dates": "даты неизвестны",
      "name": "Пётр Петрович Повалюхин",
      "notes": [
        "Отец Игоря Петровича Повалюхина."
      ],
      "parents": [],
      "partners": [
        "vera-povalyukhina"
      ]
    },
    "sasha-fateev": {
      "children": [],
      "dates": "13.05.2016",
      "name": "Саша Фатеев",
      "notes": [
        "Ребёнок Павла Александровича Фатеева и Веры Николаевны Плиткиной."
      ],
      "parents": [
        "vera-plitkina",
        "pavel-fateev"
      ],
      "partners": []
    },
    "sergey-kuteinikov": {
      "children": [
        "anna-kuteinikova"
      ],
      "dates": "даты неизвестны",
      "name": "Сергей Иванович Кутейников",
      "notes": [
        "Отец Анны Сергеевны Кутейниковой."
      ],
      "parents": [],
      "partners": [
        "maria-kuteinikova"
      ]
    },
    "tamara-grigoryeva": {
      "children": [
        "anatoly-povalyukhin",
        "oleg-povalyukhin"
      ],
      "dates": "24.12.1946",
      "memoirs": [
        "семейные-хроники",
        "отец",
        "родители"
      ],
      "name": "Тамара Николаевна Григорьева / Повалюхина",
      "notes": [
        "Жена Игоря Петровича Повалюхина."
      ],
      "parents": [
        "natalya-kureneva",
        "nikolay-grigoryev"
      ],
      "partners": [
        "igor-povalyukhin"
      ]
    },
    "tamara-zhuleva": {
      "children": [
        "anna-fateeva"
      ],
      "dates": "28.11.1958",
      "name": "Тамара Антоновна Жулёва",
      "notes": [
        "Жена Константина Александровича Фатеева."
      ],
      "parents": [],
      "partners": [
        "konstantin-fateev"
      ]
    },
    "vera-povalyukhina": {
      "children": [
        "igor-povalyukhin"
      ],
      "dates": "даты неизвестны",
      "name": "Вера Васильевна Повалюхина",
      "notes": [
        "Жена Петра Петровича Повалюхина.",
        "Мать Игоря Петровича Повалюхина."
      ],
      "parents": [],
      "partners": [
        "petr-povalyukhin"
      ]
    },
    "vera-kureneva": {
      "children": [
        "elena-podkovtseva"
      ],
      "dates": "18.08.1923 – 20.10.2017",
      "memoirs": [
        "воспоминания-веры-дмитриевны-подковцевой",
        "русская-печь"
      ],
      "name": "Вера Дмитриевна Куренева",
      "notes": [
        "Жена Николая Павловича Подковцева."
      ],
      "parents": [
        "anna-kuteinikova",
        "dmitry-kurenev"
      ],
      "partners": [
        "nikolay-podkovtsev"
      ]
    },
    "vera-plitkina": {
      "children": [
        "sasha-fateev"
      ],
      "dates": "23.04.1986",
      "name": "Вера Николаевна Плиткина",
      "notes": [
        "Жена Павла Александровича Фатеева."
      ],
      "parents": [],
      "partners": [
        "pavel-fateev"
      ]
    },
    "vera-solovyova": {
      "children": [
        "natalya-povalyukhina"
      ],
      "dates": "20.08.1968",
      "name": "Вера Дмитриевна Соловьёва",
      "notes": [
        "Жена Олега Игоревича Повалюхина."
      ],
      "parents": [],
      "partners": [
        "oleg-povalyukhin"
      ]
    }
  },
  "roots": [
    "sergey-kuteinikov"
  ]
};
